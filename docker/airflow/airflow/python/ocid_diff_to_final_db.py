...
def get_args():
    ...
    """
    parser = argparse.ArgumentParser(description='Spark Job filters db and stores it to final_db onto HDFS as well to mysql')
    ...
    parser.add_argument('--hdfs_source_dir', help='HDFS source directory, e.g. /user/hadoop/ocid/raw', required=True, type=str)
    parser.add_argument('--hdfs_target_dir', help='HDFS target directory, e.g. /user/hadoop/ocid/final', required=True, type=str)
...
if __name__ == '__main__':
    ...
    spark = SparkSession(sc).config("spark.driver.extraClassPath", "/home/hadoop/spark/jars/mysql-connector-java-8.0.27.jar")
	...
	df_diff=df_diff.filter(col("lat") >= 46.959149).filter(col("lon") >= 5.704355).filter(col("lat") <= 55.019144).filter(col("lon") <= 15.186580)
	...
	df_diff.repartition('radio').write.format("parquet").mode("append").option("path", args.hdfs_target_dir).partitionBy("radio").saveAsTable("default.lat_lon_range")

...
if __name__ == '__main__':
	...
	#germany only
	df_full=df_full.filter(col("lat") >= 46.959149).filter(col("lon") >= 5.704355) \
		.filter(col("lat") <= 55.019144).filter(col("lon") <= 15.186580)
	...
	df_full.repartition('radio').write.format("parquet").mode("append").option("path", args.hdfs_target_dir) \
	.partitionBy("radio").saveAsTable("default.lat_lon_range")
